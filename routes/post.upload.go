package routes

import (
	"log"
	"os"
	"path/filepath"
	"vspt/utils"

	"github.com/google/uuid"
	"github.com/valyala/fasthttp"
)

// PostUpload handles POST requess to the /upload route.
func PostUpload(ctx *fasthttp.RequestCtx) {
	log.Println("Incoming request")

	// Download the file to the downloaded/ directory
	fromPath, err := utils.Download(ctx)
	if err != nil {
		ctx.Error(err.Error(), fasthttp.StatusBadRequest)
		return
	}

	// Generate a UUId
	vidUUID := uuid.New().String()
	toPath, err := filepath.Abs("./transcoded/" + vidUUID + ".mp4")
	if err != nil {
		ctx.Error(err.Error(), fasthttp.StatusInternalServerError)
		return
	}

	// Transcode the downloaded file to the trasncoded/ directory
	err = utils.Transcode(fromPath, toPath)
	if err != nil {
		ctx.Error(err.Error(), fasthttp.StatusInternalServerError)
		return
	}

	// It's done transcoding, so delete the original file
	err = os.Remove(fromPath)
	if err != nil {
		log.Println("Couldn't remove uploaded file")
		ctx.Error(err.Error(), fasthttp.StatusInternalServerError)
		return
	}

	// Serve the transcoded video
	ctx.SetContentType("video/mp4")
	ctx.SendFile(toPath)
}
